...
class LAP(object):
	def __init__(
		...
		self.weights = np.zeros((max_size, reward_dim))
	...
	def add(self, state, action, next_state, reward, weights, done):
	...
	def sample(self):
		...
		return (
			...
			torch.tensor(self.weights[self.ind], dtype=torch.float, device=self.device),
