...
class Critic(nn.Module):
	def __init__(self, state_dim, action_dim, reward_dim, zs_dim=256, hdim=256, activ=F.elu):
...
class Agent(object):
	def __init__(self, state_dim, action_dim, reward_dim, max_action, offline=False, hp=Hyperparameters()):
		...
		self.critic = Critic(state_dim, action_dim, reward_dim, hp.zs_dim, hp.critic_hdim, hp.critic_activ).to(self.device)
		...
		self.replay_buffer = buffer.LAP(state_dim, action_dim, reward_dim, self.device, hp.buffer_size, hp.batch_size,
	...
	def train(self):
		...
		state, action, next_state, reward, weights, not_done = self.replay_buffer.sample()
		...
		with torch.no_grad():
			...
			Q1_target, Q2_traget = self.critic_target(next_state, next_action, fixed_target_zsa, fixed_target_zs)
			Q_target = torch.min(Q1_target, Q2_traget)
		...
		Q1, Q2 = self.critic(state, action, fixed_zsa, fixed_zs)

		td_loss = torch.cat([(Q1 - Q_target).abs(), (Q2 - Q_target).abs()], 1)
		...
		if self.training_steps % self.hp.policy_freq == 0:
			...
			Q1, Q2 = self.critic(state, actor, fixed_zsa, fixed_zs)
			wQ1, wQ2 = torch.sum(Q1*weights, dim=1), torch.sum(Q2*weights, dim=1)

			actor_loss = -0.5 * (wQ1.mean() + wQ2.mean())
			#if self.offline:
